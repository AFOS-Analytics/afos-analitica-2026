...
- **Volume + liquidity USD inline obrigatórios na Seção 1** — protocolo firmado 17/Mai e expandido 21/Mai (`feedback_afos_daily_volume_polymarket.md`): toda menção de candidato/mercado em Seção 1 cita % E volume acumulado E liquidity atual no formato canônico `XX,XX% (USD X,XXM vol · USD Xk liq)`. Aplicar inline aos **top 5 candidatos do presidencial** (Lula, Flávio, Renan + 2 da terceira via dominante do dia) e ao **total do mercado** em cada sub-mercado (2L, 3L, STF impeach, Senado, inflação). Reforça "dinheiro real" duas vezes: volume é fluxo histórico acumulado, liquidity é dinheiro disponível AGORA pra mover preço — spike com vol alto mas liq baixa = distorção (ex: MDB Senado 16,55% vol USD 254k liq USD 0 = colapsou para 4,70% em 18h). **Validator W7 emite warning se < 4 menções USD na Seção 1**. Daily 18/Mai é o gabarito do protocolo só-volume (12 USDs); dailies 19-21/Mai regrediram — não repetir. Liquidity expostas pelo proxy AFOS via `liquidityNum` desde 21/Mai/2026 commit 27e7f42.
