 ...
 *   npx tsx scripts/ingest-tse-local.ts --rede [--apply]      # baixa daqui (22/Ago)
 *
 * ⭐ O MODO `--rede`, acrescentado em 22/Ago/2026 depois de uma medição que
 * corrige o diagnóstico desta ficha: o corte da borda NÃO é entre navegador e
 * robô. Do IP de uma casa, o cliente do Node BAIXA o ZIP inteiro (HTTP 206,
 * 649 pesquisas); o `curl` apanha de qualquer lugar; e qualquer cliente vindo
 * de faixa de DATACENTER apanha, que é por isso que Vercel e GitHub Actions
 * ficam de fora. O que decide é a origem de rede somada à assinatura do
 * cliente.
 *
 * ⛔ Continua NÃO sendo contorno: usa o `fetch` do Node como está, o mesmo
 * cliente do cron, sem forjar user-agent, sem repetir tentativa e sem trocar
 * rota. A única diferença é de onde a chamada sai. Se um dia o modo `--rede`
 * também der 403, a resposta é usar o modo ARQUIVO, não insistir.
 *
...
async function main() {
  ...
  const pelaRede = args.includes('--rede')

  if (!caminho && !pelaRede) {
    console.error('❌ Falta o caminho do ZIP (ou --rede).\n   npx tsx scripts/ingest-tse-local.ts pesquisa_eleitoral_2026.zip [--apply]\n   npx tsx scripts/ingest-tse-local.ts --rede [--apply]')
    process.exit(1)
  }

  let polls
  if (pelaRede) {
    // Mesmo caminho de rede do cron, mesma função, mesmo cliente. Se a borda
    // recusar daqui também, o erro sobe e o modo ARQUIVO continua disponível.
    console.log('\n🌐 modo REDE: baixando do TSE com o mesmo cliente do cron…')
    try {
      polls = await fetchTSEPolls(ANO)
    } catch (err) {
      console.error(`❌ A borda recusou também daqui: ${err instanceof Error ? err.message : String(err)}`)
      console.error('   Use o modo ARQUIVO: baixe o ZIP no navegador e passe o caminho. NÃO insistir na rede.')
      process.exit(1)
    }
  } else {
