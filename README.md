...
**A disappearance in a single read is not a withdrawal, and the specificity of a claim is not evidence for it (10/Sep/2026).** Brazil's public electoral-poll registry is a file served through a content delivery network, and we read the whole of it every round. One afternoon round returned eight hundred and fifty-one registrations. About an hour later, mirroring the dataset, it returned **eight hundred and sixty-three**: thirteen more, all registered the previous day, and **one fewer**. This house has a true and strong rule about exactly that, namely that the court withdraws polls from its archive while our database only ever adds, and it is that rule which makes a disappearance look like meaning. I went to find what had gone, found the registration with its pollster, its office, its fieldwork dates and a release scheduled for that very day, and published the withdrawal as fact, in the dataset, in the memory note and in the commit message. **There was no withdrawal.** The next read returned eight hundred and fifty-one again, and a set comparison against the day's first read showed **zero differences in either direction**: the registration was there, and had been all along. What the rule was missing is its other half, and it is cheap: **a withdrawal is only a withdrawal if it reproduces on a second read**, because a source behind a delivery network hands out two different snapshots of the same file minutes apart, and one read cannot separate an event from an edge inconsistency. It is the same shape as the lock this house already applies to the prediction market, where two readings that agree are a price and two that disagree are a book in transit; the difference is that there the rule was written down and here it was not. What caught the error was not attention, it was an instrument written hours earlier the same day: the mirror sequence now prints each artifact's row count before and after, and the line saying the registry had gone from eight hundred and sixty-three to eight hundred and fifty-one forced a second look. Without it the public dataset would carry thirteen registrations the source does not serve and lack one that it does, and our own memory would hold an invented withdrawal with a name attached. **I had the pollster, the office, the company registration and the fieldwork dates of something that never happened**, and it was precisely that level of detail that made the claim sound well researched.
