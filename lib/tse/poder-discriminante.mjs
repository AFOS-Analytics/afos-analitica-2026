...
/**
 * Está no calendário que o painel publica: em campo agora, ou divulgação à
 * frente, **e o TSE ainda tem o registro**.
 *
 * 🔴⭐ A TERCEIRA CONDIÇÃO É DE 19/Set/2026, e ela faltava. Até aqui a pergunta
 *    era só de DATA, e a data vem do nosso banco, que nunca apaga. O TSE
 *    RETIRA. Medido no dia: a `BR005482026` da Real Time, nacional e com
 *    divulgação em 23/Set, saiu do registro do TSE na mesma rodada em que este
 *    conferidor a imprimiu como `NO CALENDÁRIO VIVO`, e o veredito fechou em
 *    "2 graves, 2 no calendário vivo" quando o vivo era **1**.
 *
 * 🔑 O custo não é o código de saída, é a FRASE. Um fantasma não pode fazer o
 *    portão calar (ele só inventa alarme, nunca esconde), mas "dois rótulos
 *    nacionais no calendário vivo" é uma afirmação sobre o mundo, e ela estava
 *    errada. É o mesmo par de leitores do relatório, um com o filtro de
 *    fantasma e outro sem, que já mordeu em `feedback_o_registro_do_tse_perde_
 *    linhas_e_o_banco_nunca_perde`.
 *
 * ⚠️ `fantasmas` ausente NÃO é "nenhum fantasma": é desconhecido, e aí a conta
 *    volta a ser só de data. Errar para o lado do alarme é o lado seguro, e
 *    quem chama tem de DIZER que mediu sem o conjunto.
 *
 * @param {object} reg
 * @param {string} hoje  AAAA-MM-DD, data civil do Brasil
 * @param {Set<string>|null} [fantasmas]  protocolos que saíram do registro do TSE
 */
export function noCalendarioVivo(reg, hoje, fantasmas = null) {
  if (fantasmas instanceof Set && fantasmas.has(protocoloDe(reg))) return false
 ...
 * @param {{hoje: string, fantasmas?: Set<string>|null}} opcoes
 *   `hoje` em ISO 'AAAA-MM-DD', injetado para o teste.
 *   `fantasmas` são os protocolos que saíram do registro do TSE. Ausente quer
 *   dizer DESCONHECIDO, não "nenhum": ver `noCalendarioVivo`.
 * @returns {{ok, poder, achados, graves, vivos, comFantasmas}}
 */
export function conferirEscopoDerivado(registros, { hoje, fantasmas = null } = {}) {
  if (!hoje) throw new Error('conferirEscopoDerivado exige `hoje` em AAAA-MM-DD')
  if (fantasmas != null && !(fantasmas instanceof Set)) {
    // 🕳️ Anti-silêncio de TIPO: um array passado aqui faria `.has` explodir, mas
    // um objeto qualquer faria `instanceof` dar falso e o conjunto sumiria
    // CALADO, que é o defeito voltando por outra porta.
    throw new Error('conferirEscopoDerivado: `fantasmas` precisa ser um Set ou null')
  }
  ...
  for (const reg of registros) {
    ...
    achados.push({
      ...
      // `retirado` separa as duas razões de NÃO estar vivo, que pedem frases
      // opostas: "já vencido" é dívida de dataset, "já retirado" é compromisso
      // que o TSE desfez e que ninguém precisa conferir na divulgação.
      retirado: fantasmas instanceof Set && fantasmas.has(protocoloDe(reg)),
      vivo: noCalendarioVivo(reg, hoje, fantasmas),
    })
  }
  ...
  // E desde 19/Set/2026 registro RETIRADO pelo TSE também não chega à tela.
  return { ok: vivos.length === 0, poder, achados, graves, vivos, comFantasmas: fantasmas instanceof Set }
}
