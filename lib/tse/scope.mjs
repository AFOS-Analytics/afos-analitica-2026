...
// `unidade da federacao (<estado>)`: forma usada pela Veritá, que declara o universo
// como "eleitorado da respectiva Unidade da Federação (Amazonas)". O "respectiva" no
// meio quebra o padrão `eleitor(es|ado) d[eoa] <estado>`, e as 4 rodadas de 27/Jul
// (AM, PA, PR, AP) caíram em unknown. Exige SINGULAR e o nome do estado entre
// parênteses: o plural `unidades da federacao` continua sendo sinal NACIONAL, e sem os
// parênteses não casa, o que impede falso positivo em boilerplate de plano amostral.
const STATE_UNIVERSE = new RegExp(
  `(estado d[eoa] (${STATE_NAMES}))|(eleitor(es|ado) d[eoa] (${STATE_NAMES}))|` +
  `(eleitorado d[eoa] (${UF_ABBREV})\\b)|` +
  `(unidade da federacao[^.(]{0,5}\\((${STATE_NAMES})\\))|` +
