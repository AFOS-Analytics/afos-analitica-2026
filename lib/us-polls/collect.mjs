...
import { ehOutroInstrumento, motivoDoInstrumento } from './instrumento-medido.mjs'
...
export function media(pesquisas, dias, agora = new Date(), registroCasas = undefined, registroInstrumento = undefined) {
  ...
  // 🎯 A média se chama generic ballot, então ela mede generic ballot. Rodada de
  // onda declarada como OUTRO instrumento sai da conta, por decisão do André em
  // 26/Set/2026, e o registro é `instrumento-medido.mjs`, com prova e data.
  //
  // ⛔ Ela NÃO some: sai listada em `excluidasPorInstrumento`, com motivo e
  // prova, para a conta ser refeita por quem ler o arquivo. Exclusão calada
  // seria pior que a inclusão errada.
  const fora = naJanela.filter((p) => ehOutroInstrumento(p, registroInstrumento))
  const elegiveis = naJanela.filter((p) => !ehOutroInstrumento(p, registroInstrumento))
  ...
  // Mesma hierarquia de recorte das que entram: a lista de excluídas fala em
  // RODADAS, para "38 menos 3 dá 35" ser lido direto, e não em linhas soltas.
  const foraPorRodada = new Map()
  for (const p of fora) {
    const chave = `${canon(p.instituto)}|${p.campoFim}`
    const atual = foraPorRodada.get(chave)
    const peso = (x) => [ORDEM_RECORTE[x.amostraTipo] ?? 0, x.amostra ?? 0, ...ordemDeEmpate(x, registroCasas)]
    if (!atual || compararEmpate(peso(p), peso(atual)) > 0) foraPorRodada.set(chave, p)
  }
  ...
  return {
    ...
    // 📌 Uma entrada por rodada excluída, com o porquê ao lado.
    excluidasPorInstrumento: [...foraPorRodada.values()]
      .map((p) => {
        const m = motivoDoInstrumento(p, registroInstrumento)
        return {
          instituto: canon(p.instituto),
          campoInicio: p.campoInicio ?? null,
          campoFim: p.campoFim,
          amostraTipo: p.amostraTipo ?? null,
          dem: p.dem,
          rep: p.rep,
          desde: m?.desde ?? null,
          medidoEm: m?.medidoEm ?? null,
          ressalva: m?.ressalva ?? null,
          prova: m?.prova ?? null,
        }
      })
      .sort((a, b) => b.campoFim.localeCompare(a.campoFim) || a.instituto.localeCompare(b.instituto)),
    ...
    metodo: 'média aritmética simples, sem ponderação e sem exclusão de instituto. Quando o mesmo instituto publica mais de um recorte da mesma rodada, entra um só, pela hierarquia provável votante > eleitor registrado > adultos. Quando o índice lista a mesma casa sob dois nomes, o de quem executa e o de quem publica, ou duas grafias, ela conta uma vez, pelo nome de quem executa, por tabela declarada casa a casa. Onda que o instituto publicou com OUTRO instrumento, como cédula com nomes de candidatos, fica de fora e é listada em excluidasPorInstrumento com a prova.',
