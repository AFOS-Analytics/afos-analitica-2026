...
export function media(pesquisas, dias, agora = new Date()) {
...
export async function coletarGenericBallot({ dias = 30, agora = new Date() } = {}) {
  ...
  for (let tentativa = 1; tentativa <= 3; tentativa++) {
    try {
      res = await fetch(URL_RAW, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(20_000) })
      if (res.ok) break
      ultimoErro = new Error(`Wikipédia devolveu HTTP ${res.status}`)
    } catch (e) {
      ultimoErro = e
    }
    if (tentativa < 3) await new Promise((r) => setTimeout(r, 2000 * tentativa))
  }
  if (!res || !res.ok) throw ultimoErro || new Error('Wikipédia não respondeu')
