...
import { deveRodada } from './instrumento-medido.mjs'
...
function campoEm(todas, iso, janelaDias, excluirInstituto) {
  ...
  const na = todas.filter((r) => serieDaCasa(r.instituto) !== excluirInstituto && r.campoFim <= iso && r.campoFim >= de)
...
function efeitoDaCasa(todas, instituto, desde, janelaDias) {
  const minhas = todas.filter((r) => serieDaCasa(r.instituto) === instituto && r.campoFim >= desde)
  const desvios = []
...
export function medirExposicao(dados, cadencia, opts = {}) {
  ...
  const semEfeito = []
  // rodadas que a cadência projetava e que a casa NÃO deve, por instrumento medido
  const naoDevidas = []

  for (const c of cadencia?.atrasadas ?? []) {
    const minhas = todas.filter((r) => serieDaCasa(r.instituto) === c.instituto)
    ...
    // 🔬 E das que caem na janela, saem as que a casa NÃO deve, porque o
    //    instrumento dela foi MEDIDO como outro naquela data.
    //
    // 🔴 Isto foi diagnosticado em 15/Set/2026 e ficou só na memória: a
    //    exposição seguia cobrando da The Economist/YouGov rodadas de uma
    //    cédula NOMINAL, que não é generic ballot. Em 22/Set eram três ondas
    //    seguidas, e elas moviam a linha de D+7.00 para D+6.56.
    //
    // 🔑 O corte é por ONDA e não por casa: a mesma casa segue devendo as
    //    rodadas anteriores à troca, quando ela media a mesma coisa. Excluir a
    //    casa inteira apagaria buraco real do passado.
    const naJanelaBruta = esperadas.filter((d) => d >= base.desde)
    const naJanela = []
    for (const d of naJanelaBruta) {
      const v = deveRodada(c.instituto, d)
      if (v.deve) naJanela.push(d)
      else naoDevidas.push({ instituto: c.instituto, campoFim: d, motivo: v.motivo })
    }
    // ⛔ Casa que não deve NENHUMA rodada na janela sai da lista de cobrança,
    //    mas NÃO some do relatório: ela aparece em `naoDevidas`, com o motivo.
    //    Sumir com a linha esconde a decisão; declará-la a expõe.
  ...
  return {
    ...
    naoDevidasPorInstrumento: naoDevidas,
