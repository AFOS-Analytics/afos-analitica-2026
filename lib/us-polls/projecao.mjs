...
import { ehOutroInstrumento } from './instrumento-medido.mjs'
 ...
 */
export function mediaEm(polls, diaIso, dias, registros = {}) {
  if (!Array.isArray(polls) || !ehDataIso(diaIso)) return null
  // 🔑 Os registros ATRAVESSAM. Sem isto, o `n` desta projeção usa o registro
  // padrão e a coluna "quem SAI" usa o que o chamador passou, e as duas colunas
  // da mesma tabela deixam de falar da mesma média. Em produção os dois são o
  // mesmo objeto, então o defeito é invisível ali: quem o achou foi a asserção
  // "nenhum dia perde n sem nomear quem saiu", em 26/Set/2026.
  return media(polls, dias, new Date(diaIso + HORA_DE_REFERENCIA), registros.registroCasas, registros.registroInstrumento)
}
...
export function projetarJanela(dados, opts = {}) {
  ...
  for (let i = 0; i <= horizonte; i++) {
    ...
    const m = mediaEm(polls, dia, dias, opts)
    if (!m) {
      esvaziaEm = dia
      linhas.push({ dia, desde: somarDias(dia, -dias), vazia: true, media: null, saindo: [], saindoForaDaMedia: 0 })
      break
    }
    ...
    if (corteAnterior !== null) {
      ...
      for (const p of polls) {
        ...
        // 🎯 Rodada que NÃO ENTRAVA na média não explica queda nenhuma, e esta
        // coluna existe para explicar a queda. Medido em 26/Set/2026, no dia em
        // que a exclusão por instrumento entrou: a projeção nomeava a onda de
        // 08/Set da The Economist/YouGov saindo em 09/Out e o `n` NÃO caía,
        // porque aquela onda nunca esteve na conta. É o mesmo raciocínio do
        // comentário acima, de não listar três recortes de uma rodada só.
        //
        // ⛔ E a omissão não é silenciosa: ela é CONTADA e sai declarada, porque
        // sumir com a linha esconderia que a rodada deixa a janela de verdade.
        if (ehOutroInstrumento(p, opts.registroInstrumento)) {
          foraDaMedia.add(`${serieDaCasa(p.instituto)}|${p.campoFim}`)
          continue
        }
        // 🧬 Pela SÉRIE, como a média: casa escrita sob dois nomes sairia da janela
        // como duas rodadas, e a projeção diria que perde o dobro do que perde.
        const serie = serieDaCasa(p.instituto)
        const chave = `${serie}|${p.campoFim}`
        const j = porRodada.get(chave)
        if (j) j.linhas++
        else porRodada.set(chave, { instituto: serie, campoFim: p.campoFim, linhas: 1 })
      }
      saindo.push(...porRodada.values())
    }
    linhas.push({ dia, desde: m.desde, vazia: false, media: m, saindo, saindoForaDaMedia: foraDaMedia.size })
  ...
  return {
    ...
    base: {
      ...
      mediaHoje: mediaEm(polls, dia0, dias, opts),
    },
    linhas,
