...
/** Papéis aceitos. Cada um é um jeito diferente de o nome divergir, e cobra prova diferente. */
export const PAPEIS = {
  ...
  // A mesma casa escrita de outro jeito na MESMA onda: maiúscula, palavra a
  // menos, sigla. Pede a assinatura da rodada e os valores.
  grafia: 'mesma casa, grafia diferente, na mesma onda',
  // O índice trocou o nome da casa ENTRE ondas. Não existe rodada em que os
  // dois nomes apareçam, então não há assinatura a exigir.
  serie: 'o índice trocou o nome da casa entre ondas',
}
...
export const CASAS_DECLARADAS = [
  {
    ...
    variantes: [
      ...
      },
      {
        ...
        papel: 'serie',
        prova: { onde: 'post da casa de 16/Set/2026, conferido na rodada daquele dia' },
        motivo: 'o patrocínio passou a ser um projeto de financiamento coletivo e entrou no rótulo.',
      },
    ],
  },
  {
    serie: 'Focaldata/Financial Times',
    conferidoEm: '2026-08-01',
    motivo: 'Apelido já tratado como a mesma série dentro do conferidor de defasagem desde agosto.',
    // ⚠️ INCONSISTÊNCIA DECLARADA, e não silenciada: esta entrada canoniza para o
    // rótulo que CARREGA o veículo, ao contrário da régua de 25/Set, em que fica
    // o nome de quem executa. Inverter renomearia a série que três medidores já
    // usam (defasagem, cadência e instrumento), então fica como está até haver
    // decisão, e a inconsistência fica escrita em vez de descoberta depois.
    ressalva: 'canoniza para o rótulo com o veículo, contra a régua de "fica quem executa". Pendente de decisão.',
    variantes: [
      {
        nome: 'Focaldata',
        papel: 'serie',
        prova: { onde: 'conferidor de defasagem, que trata os dois como a mesma casa desde agosto de 2026' },
        motivo: 'o índice às vezes escreve só o executor, às vezes o par com o veículo.',
      },
    ],
  },
  {
...
export function conferirCarga(registro = CASAS_DECLARADAS) {
  ...
  registro.forEach((e, i) => {
    ...
    for (const v of e.variantes) {
      ...
      // A assinatura só existe quando os dois nomes aparecem na MESMA onda.
      if (v?.papel !== 'serie') {
        if (!v?.assinatura) erros.push(`${q}: sem assinatura da rodada em que o par foi visto`)
        if (!Array.isArray(v?.valores) || v.valores.length === 0) erros.push(`${q}: sem valores, a conferência não caducaria`)
      }
