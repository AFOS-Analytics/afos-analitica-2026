 ...
 *
 * 🔑 `mesmaOnda` diz se a variante foi vista na MESMA onda que OUTRO rótulo
 * desta série, e não necessariamente o rótulo canônico: numa série com duas
 * variantes, a colisão observável pode ser entre as duas.
 *
...
export const CASAS_DECLARADAS = [
  ...
  {
    // ⚖️ A JUNÇÃO COM "Emerson College" PURO É DECISÃO DO ANDRÉ, de 26/Set/2026.
    // A primeira versão desta entrada declarava a junção como NÃO tomada, com a
    // advertência de que "Morning Consult" e "Morning Consult/Cato Institute" são
    // produtos diferentes. A decisão foi juntar, e a régua que a sustenta é a
    // mesma de 25/Set: fica o nome de quem EXECUTA, e quem vai a campo nas duas
    // é a Emerson College.
    //
    // 📊 Medido antes de aplicar: NENHUMA rodada colide, porque nenhum campoFim
    // se repete entre os rótulos. A junção custa UM instituto na contagem e zero
    // rodada, e os institutos da média caem de 27 para 26.
    //
    // ⚠️ E ela tem uma consequência que não é de contagem: a CADÊNCIA da casa
    // passa a incluir a onda de 12-15/Set, então a Emerson deixa de parecer
    // mensal em setembro. Isso é o que de fato aconteceu, a casa foi a campo
    // duas vezes no mês, e a cadência medindo isso é correto e não efeito
    // colateral.
    serie: 'Emerson College',
    conferidoEm: '2026-09-26',
    motivo:
      'A Emerson College Polling é quem vai a campo nas duas séries; a RealClear Opinion Research encomenda uma delas. E o índice escreveu a mesma rodada de 12-15/Set com e sem um ESPAÇO depois da barra, de um dia para o outro.',
    variantes: [
      {
        ...
        motivo:
          'a matéria estabelece a AUTORIA e o patrocínio da onda de 12-15/Set (instituto, n, recorte, margem e datas batem com o índice). ⛔ Ela NÃO traz a pergunta do generic ballot, e por isso o topline daquela rodada segue como dívida aberta em fonte-conferida.mjs: esta prova sustenta quem executou, não o número.',
      },
      {
        nome: 'Emerson College/ RealClear Opinion Research',
        papel: 'grafia',
        ...
        valores: ['37/29'],
        motivo: 'a diferença é um espaço depois da barra, e nada mais. A colisão observável é contra a outra variante, não contra o rótulo canônico.',
      },
    ],
  },
