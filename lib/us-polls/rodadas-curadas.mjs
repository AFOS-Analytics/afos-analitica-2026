...
export const RODADAS_CURADAS = [
  ...
  /**
   * ActiVote, as 2 ondas de agosto e setembro. ⚖️ ENTRADA POR DECISAO DO
   * ANDRE em 20/Set/2026, com a normalizacao declarada, e nao por régua.
   *
   * ✅ Pergunta, no metadado da propria pagina: "If the election for the US
   *    House of Representatives were held today, which party would you vote
   *    for?" E cedula generica na forma de PARTIDO, sem ancora de distrito:
   *    mais fraca que a da Marquette ou a da Focaldata, e nao e outro
   *    instrumento como a da Quinnipiac.
   *
   * 🔴 O QUE ESTA LINHA TEM DE DIFERENTE DE TODAS AS OUTRAS DO ARQUIVO, e e
   *    por isso que a entrada foi decisao e nao rotina: a casa publica so a
   *    divisao de DUAS VIAS NORMALIZADA EM 100%. Nao ha indeciso, nao ha
   *    terceira via, nao ha "nao vou votar". Por isso `outros: 0` e `opcoes`
   *    vazio: nao e que a decomposicao seja zero, e que ela NAO EXISTE.
   *
   * ⚠️ E normalizar NAO e neutro: tirar do denominador quem nao escolheu
   *    INFLA a margem. Um D 44 x R 37 cru vira 54,3 x 45,7 normalizado, e o
   *    vao passa de 7,0 para 8,6. Entao estas duas linhas entram na media
   *    medindo uma grandeza ligeiramente diferente da das outras 21, e isso
   *    fica escrito aqui em vez de virar diferenca sem nome na serie.
   *    ⛔ Isto NAO e a dimensao do leaner: leaner e sobre QUEM foi perguntado
   *       de novo, normalizacao e sobre QUEM saiu da conta.
   *       Ver memory/reference_par_binario_a_soma_se_move_sem_o_meio_mover.md
   *
   * ⚠️ A CASA SE CONTRADIZ EM DOIS PONTOS, e os dois ficam registrados porque
   *    nenhum e resolvivel por fora:
   *    1. o corpo da pagina diz "1000 likely midterm voters" e o metadado da
   *       MESMA pagina diz "1,000 registered voters". Entra LV, que e o que o
   *       texto humano diz e o que o indice ja gravou na onda de junho;
   *    2. na onda de 24/Ago o cartao imprime "D+3,4%" e a prosa logo abaixo
   *       diz "lead the Republicans by 3.5%". 51,7 - 48,3 = 3,4. Vale a
   *       subtracao, que e conferivel; o 3,5 fica citado e nao usado.
   *
   * 📌 DECIMAL de proposito, e nao arredondamento: as duas ondas arredondam
   *    para o MESMO 52 x 48, e ai duas medicoes distintas da casa virariam a
   *    mesma linha. O arquivo ja tem precedente de decimal na Quantus.
   *
   * 🔗 Nao ha PDF: a casa publica a crosstab como tabela HTML na propria
   *    pagina do release. A URL do release E o documento.
   */
  {
    instituto: 'Activote',
    campoInicio: '2026-08-10', campoFim: '2026-08-24',
    amostra: 1000, amostraTipo: 'LV', margemErro: 3.1,
    dem: 51.7, rep: 48.3, outros: 0, vantagemDem: 3.4,
    fontePrimaria: 'https://www.activote.net/polls/generic-ballot/2026-08-24',
    opcoes: {},
    notas: 'Duas vias NORMALIZADA em 100%, sem indeciso: a decomposicao nao existe, nao e zero. A prosa da pagina diz 3,5 e a subtracao da 3,4.',
  },
  {
    instituto: 'Activote',
    campoInicio: '2026-08-31', campoFim: '2026-09-10',
    amostra: 1000, amostraTipo: 'LV', margemErro: 3.1,
    dem: 52.3, rep: 47.7, outros: 0, vantagemDem: 4.6,
    fontePrimaria: 'https://www.activote.net/polls/generic-ballot/2026-09-11',
    opcoes: {},
    notas: 'Duas vias NORMALIZADA em 100%, sem indeciso: a decomposicao nao existe, nao e zero. Data mediana de campo 05/Set.',
  },
]
...
/**
 * 🧬 A chave é a SÉRIE da casa, não o nome escrito.
 *
 * 🔴 MEDIDO EM 25/Set/2026, e o defeito era NOSSO. A curada da ActiVote entrou
 * em 20/Set escrita `Activote`, e o índice escreve `ActiVote`. Quando o índice
 * indexou a onda de 31/Ago-10/Set, a curada NÃO se aposentou: uma letra
 * derrotou a regra "o índice sempre vence", e a casa entrou DUAS vezes na
 * média. E a rede de segurança errou pelo mesmo motivo: `suspeitas` casa por
 * instituto e recorte, e também não viu.
 */
const chave = (p) => `${serieDaCasa(p.instituto)}|${p.campoFim}|${p.amostraTipo}`
 ...
 *
 * ⚠️ E "vence" tem um preço que fica declarado: a curada da ActiVote existia para
 * guardar o DECIMAL (52,3 x 47,7) que o índice arredonda (52 x 48). Ao se
 * aposentar, ela leva o decimal com ela, e a média passa a servir o valor
 * arredondado daquela onda. Manter as duas seria manter a casa contada duas
 * vezes, e uma regra só é melhor que duas em conflito.
 */
export function mesclarCuradas(doIndice, curadas = RODADAS_CURADAS) {
  ...
  for (const c of curadas) {
    ...
    for (const p of marcadas) {
      if (
        serieDaCasa(p.instituto) === serieDaCasa(c.instituto) &&
