...
export function medirCadencia(dados, agora = new Date()) {
  ...
  for (const p of dados?.polls ?? []) {
    ...
    // A SÉRIE, não o nome cru: o índice renomeia casas (lib/us-polls/casas.mjs).
    const casa = serieDaCasa(p.instituto)
    if (!rodadasPorCasa.has(casa)) rodadasPorCasa.set(casa, new Set())
    rodadasPorCasa.get(casa).add(p.campoFim)
  }
