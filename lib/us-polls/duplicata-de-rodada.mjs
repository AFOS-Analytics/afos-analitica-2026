...
export function duplicatas(polls, registro = DUPLICATA_CONFERIDA) {
  ...
  for (const [k, linhas] of porAssin) {
    ...
    // 🧬 RESOLVIDA quando a tabela declarada de `casa-canonica.mjs` já diz que os
    // dois nomes são a MESMA casa: a média conta a casa uma vez e o par deixa de
    // ser dívida aberta. ⛔ Ele continua SAINDO no relatório, com a marca: par
    // que desaparece da tela esconde a decisão que foi tomada sobre ele.
    grupo.casaCanonica = [...new Set(nomes.map((n) => serieDaCasa(n)))]
    grupo.resolvida = grupo.casaCanonica.length === 1
    saida.push(grupo)
  }
