 ...
 *   'slug'      → a casa publica cada release numa URL com a DATA dentro
 *                 (`/polls/generic-ballot/2026-09-22/`), e a nossa própria base
 *                 cita essa URL em `fontePrimaria`. Aí a comparação é slug
 *                 contra slug, release contra release, sem converter nada.
 *
 * 🔑 POR QUE 'slug' EXISTE, medido em 26/Set/2026 na ActiVote. A página dela
 * responde, tem a frase do campo ("in the field between ...") QUEBRADA por tags,
 * então nenhum intervalo contíguo é legível, e tem as datas de release com ano
 * numa tabela. Registrar como 'meta' teria trocado "casa não está no registro"
 * por um INDETERMINADO permanente, o que renomeia o ponto cego em vez de fechá-lo.
 *
 * ⛔ E a comparação é pelo MÁXIMO, nunca por diferença de conjuntos. A página da
 * ActiVote lista o release de 29/Jun, que a nossa base TEM, citado por uma URL de
 * outra forma, de antes de a casa adotar o padrão datado. Diferença de conjuntos
 * inventaria um buraco de junho a cada passada.
 */
export const LISTAGENS_POR_CASA = {
  ...
  /**
   * ⚖️ REGISTRADA EM 26/Set/2026, por decisão do André, porque a casa era ponto
   * cego: ela entrou na cadência quando as curadas dela se aposentaram, e saía
   * como "nao esta no registro" em toda passada.
   *
   * 📌 A raiz serve a release MAIS RECENTE, não um índice: o corpo traz o texto
   * da onda do dia e uma tabela "Every reading" com as anteriores, cada uma
   * linkada por uma URL datada. São essas URLs que este formato lê.
   */
  ActiVote: {
    host: 'www.activote.net',
    url: 'https://www.activote.net/polls/generic-ballot/',
    formato: 'slug',
    // 🔑 Classe de caractere, e nunca a forma com barra invertida: o padrao vive
    // numa STRING, e barra invertida em string e a armadilha que ja morde esta
    // casa (o heredoc a colapsa, e a string a engole como caractere solto).
    slug: 'generic-ballot/(20[0-9]{2}-[0-9]{2}-[0-9]{2})',
    nota: 'a frase do campo ("in the field between ...") e quebrada por tags, entao nenhum intervalo contiguo e legivel; a data forte aqui e o slug do release',
  },
  ...
  // A chave é a SÉRIE (lib/us-polls/casas.mjs): o índice listou a rodada de
  // 13-15/Set/2026 como "Big Data Poll/Public Polling Project", sem o "(R)".
  'Big Data Poll': {
    host: 'www.bigdatapoll.com',
    url: null,
    formato: null,
    /**
     * Medido em 07/Set/2026, e o resultado e util pelo lado negativo.
     *
     * A pagina do tracker EXISTE, em /project/generic-ballot/, e nao serve para
     * este leitor: ela nao tem UMA data no corpo. A serie mora num grafico
     * datawrapper (5I4Rz), cujo conjunto de dados e legivel em
     * datawrapper.dwcdn.net/5I4Rz/<versao>/dataset.csv.
     *
     * ✅ Na versao 23, a mais alta que responde, o ultimo ponto e 7/29/2026, D
     * 48.7 x R 38.1, que e EXATAMENTE a rodada que o indice ja tem. Ou seja: nao
     * ha rodada dela faltando, e o tracker e mensal, entao agosto esta em atraso
     * na origem e nao na nossa coleta.
     */
    nota: '/polls devolve 404; /project/generic-ballot/ existe mas nao tem data no corpo. A serie vive no grafico datawrapper 5I4Rz, e em 07/Set/2026 o ultimo ponto dela era 29/Jul/2026, a mesma rodada que ja esta no indice',
  },
  'Reuters/Ipsos': {
    host: 'www.ipsos.com',
...
/**
 * As datas de RELEASE que a página cita nas próprias URLs.
 *
 * `padrao` é string e não RegExp de propósito: RegExp com `g` guarda estado
 * entre chamadas, e registro é dado, não objeto vivo.
 */
export function extrairSlugsDeRelease(texto, padrao) {
  if (typeof texto !== 'string' || !padrao) return []
  const re = new RegExp(padrao, 'g')
  const isos = new Set()
  for (const m of texto.matchAll(re)) if (m[1]) isos.add(m[1])
  return [...isos].sort().reverse()
}
...
export async function verificarCasa(instituto, ultimoCampoNoIndice, opts = {}) {
  ...
  if (reg.formato === 'slug') {
    const naPagina = extrairSlugsDeRelease(texto, reg.slug).filter((iso) => iso <= hoje)
    if (!naPagina.length) {
      return {
        ...base,
        veredito: VEREDITOS.INDETERMINADO,
        detalhe: 'listagem sem URL de release datada (formato mudou ou e montada no cliente)',
      }
    }
    // ⛔ Sem release conhecido na nossa base não há o que comparar, e isso NÃO é
    // "nada novo": é o mesmo silêncio que o cabeçalho deste arquivo proíbe.
    const conhecidos = (opts.slugsConhecidos ?? []).filter((iso) => typeof iso === 'string')
    if (!conhecidos.length) {
      return {
        ...base,
        veredito: VEREDITOS.INDETERMINADO,
        ultimoNaListagem: naPagina[0],
        detalhe: `a pagina declara release ate ${naPagina[0]} e a nossa base nao cita release nenhum desta casa: nada a comparar`,
      }
    }
    const nosso = [...conhecidos].sort().reverse()[0]
    const novos = naPagina.filter((iso) => iso > nosso)
    if (novos.length) {
      return {
        ...base,
        veredito: VEREDITOS.RODADA_FORA_DO_INDICE,
        ultimoNaListagem: naPagina[0],
        rodadasFora: novos.map((iso) => ({ iso, evidencia: `release ${iso} na listagem, o nosso mais novo e ${nosso}` })),
        detalhe: `${novos.length} release(s) mais novo(s) que o nosso mais recente (${nosso})`,
      }
    }
    return {
      ...base,
      veredito: VEREDITOS.SEM_RODADA_NOVA,
      ultimoNaListagem: naPagina[0],
      detalhe: `o release mais novo da listagem (${naPagina[0]}) ja esta citado na nossa base`,
    }
  }
...
export async function verificarCasasAtrasadas(dados, cadencia, opts = {}) {
  ...
  for (const p of dados?.polls ?? []) {
    ...
    const casa = serieDaCasa(p.instituto)
    const atual = ultimoPorCasa.get(casa)
    if (!atual || p.campoFim > atual) ultimoPorCasa.set(casa, p.campoFim)
  }

  // 🔑 Os slugs de release que a NOSSA base cita, por casa: é o outro lado da
  // comparação do formato 'slug', e ele só existe aqui, onde os polls estão.
  const slugsPorCasa = new Map()
  for (const p of dados?.polls ?? []) {
    if (!p?.instituto || !p?.fontePrimaria) continue
    const casa = serieDaCasa(p.instituto)
    const reg = LISTAGENS_POR_CASA[casa]
    if (reg?.formato !== 'slug' || !reg.slug) continue
    for (const iso of extrairSlugsDeRelease(p.fontePrimaria, reg.slug)) {
      if (!slugsPorCasa.has(casa)) slugsPorCasa.set(casa, [])
      slugsPorCasa.get(casa).push(iso)
    }
  }
  ...
  for (const c of cadencia?.atrasadas ?? []) {
    resultados.push(
      await verificarCasa(c.instituto, ultimoPorCasa.get(c.instituto) ?? null, {
        ...opts,
        slugsConhecidos: opts.slugsConhecidos ?? slugsPorCasa.get(c.instituto) ?? [],
      })
    )
  }
