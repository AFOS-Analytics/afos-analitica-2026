...
import { usePrefersReducedMotion } from '../hooks/useFocusTrap';
...
export function DashboardGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (showGate && !impressionTracked.current) {
      impressionTracked.current = true;
      trackEvent('gate_impression', { source: 'gate' });
    }
  }, [showGate]);

  // Focus first input do gate quando aparece (a11y)
  useEffect(() => {
    if (showGate && dialogRef.current) {
  ...
  return (
    <>
      ...
      <div
        style={showGate ? { filter: 'blur(8px)', pointerEvents: 'none', userSelect: 'none' } : undefined}
      ...
      {showGate && (
        ...
        >
          <div
            ...
            style={{ animation: reducedMotion ? undefined : 'afosSlideUp 0.4s ease-out' }}
          >
            <div className="h-1.5 bg-gradient-to-r from-primary to-blue-400" aria-hidden="true" />
            <div className="px-6 py-8 sm:px-8">
              <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mx-auto mb-5" aria-hidden="true">
              ...
              <h2 id="gate-title" className="text-dark font-bold text-xl text-center leading-snug mb-2">{t('gate.title')}</h2>
              <p id="gate-description" className="text-gray-700 text-sm text-center leading-relaxed mb-5">{t('gate.description')}</p>
              <ul className="flex flex-col gap-2 mb-6 list-none p-0">
                {(['benefit1', 'benefit2', 'benefit3'] as const).map((key) => (
                  <li key={key} className="flex items-center gap-2.5">
                    <div className="w-5 h-5 rounded-full bg-green-50 flex items-center justify-center flex-shrink-0" aria-hidden="true">
