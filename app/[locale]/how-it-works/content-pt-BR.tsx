...
export function HowItWorksPtBR() {
  return (
    <>
      ...
      <h2 id="landing" className={S.h2}>A página inicial (landing)</h2>
      ...
      <Card title="Como o AFOS Daily funciona">
        ...
        <p><strong>Arquivo de edições:</strong> a página <code>/daily</code> lista <strong>todas as edições publicadas</strong>, agrupadas por mês e com a mais recente em destaque, com um seletor para <strong>ir direto a uma data</strong> e os mesmos seletores de <strong>idioma</strong> (PT · EN · ES) e <strong>tema</strong> (claro / Sapphire Blue). Dentro de cada síntese, os botões <strong>"← Síntese anterior"</strong> e <strong>"Próxima síntese →"</strong> e o link <strong>"Todas as edições"</strong> levam ao arquivo.</p>
      ...
      <Card title="Como o AFOS Tradeoff funciona">
        ...
        <p><strong>3 idiomas, permalink e arquivo:</strong> PT-BR · EN · ES, cada edição com URL própria (<code>/tradeoff/br/2026-06-01</code>). A página <code>/tradeoff</code> é o <strong>arquivo de edições</strong>: lista todas (por número e semana), com a mais recente em destaque e seletores de <strong>idioma</strong> e <strong>tema</strong> (claro / Sapphire Blue). Dentro de cada edição, os botões de navegação e o link <strong>"Todas as edições"</strong> levam ao arquivo.</p>
      </Card>
      ...
      <h3 id="contexto-estrutural" className={S.h3Anchor}>Contexto estrutural, a moldura do país</h3>
      <p className={S.p}>Abaixo dos sinais eleitorais, cada caso validado e o dashboard do Brasil trazem um bloco de <strong>Contexto estrutural</strong>: indicadores oficiais e abertos do <strong>Banco Mundial</strong> que enquadram o país <strong>ao lado</strong> do sinal de mercado, não como previsor dele. São três grupos: <strong>Governança</strong> (seis Worldwide Governance Indicators, escala 0 a 100: estabilidade política, voz e democracia, estado de direito, efetividade do governo, qualidade regulatória, controle de corrupção), <strong>Economia</strong> (população, PIB, PIB per capita, inflação) e <strong>Educação</strong> (gasto público em educação e expectativa de anos de escola). São indicadores anuais e estruturais, mudam devagar e <strong>não preveem o resultado eleitoral</strong>, servem de pano de fundo para a leitura. Fonte: World Bank (WGI via Data360 e WDI v2), aberta e citável.</p>

      <h3 id="grafo-cruzamento" className={S.h3Anchor}>O grafo do cruzamento, o método virou um mapa navegável</h3>
      <p className={S.p}>Em cada caso validado e, sobretudo, no <strong>dashboard do Brasil</strong>, o cruzamento ganhou um <strong>grafo força-dirigido no estilo do Obsidian</strong>. A eleição fica no centro, cercada pelos quatro pilares (mercados, pesquisas, imprensa e contexto estrutural). A <strong>estrela é a divergência</strong>: cada candidato é ligado por uma linha fina, colorida pela magnitude (vermelho = alta, amarelo = média, verde = convergência), com o <strong>Δpp escrito sobre a própria linha</strong>. Nos casos validados um nó de <strong>"Resultado real"</strong> aponta em verde para quem de fato venceu; as bolas dos candidatos usam as <strong>cores partidárias</strong>.</p>
      <p className={S.p}>No dashboard, o grafo é o <strong>cérebro navegável</strong> da plataforma. Passe o mouse num nó e ele e suas conexões acendem em azul; clique e cada bola leva ao seu destino: os nós de <strong>dado</strong> abrem a pasta correspondente no <strong>dataset aberto no Hugging Face</strong>, os de <strong>contexto</strong> rolam para o card de contexto da página, e os <strong>atalhos</strong> levam aos produtos (Daily, Tradeoff, Global, White Paper, Governança), às seções do painel e ao <strong>Harvard Dataverse</strong> (coleção + DOI). É o método inteiro, dado aberto, eleição viva e plataforma, num só mapa, em três idiomas.</p>
      ...
      <p className={S.p}>Este é o rodapé do <strong>dashboard e das demais páginas</strong> (a página inicial tem uma versão mais enxuta, citada na seção da landing). Ele é organizado em <strong>quatro blocos enxutos</strong>, cada um com propósito claro. Nenhum link no footer aponta para página vazia, cada um entrega algo específico.</p>
      ...
      <ul className={S.ul}>
        ...
        <li><strong>Governança</strong>, como a integridade editorial é garantida por código (validadores automáticos + regras versionadas), não por revisão humana caso a caso</li>
        <li><strong>Trademark</strong>, política de uso do nome e da marca "AFOS Analytics"</li>
      </ul>
      ...
      <h3 className={S.h3}>Bloco 4, Redes sociais e dados abertos</h3>
      <p className={S.pTight}>Uma linha de ícones conecta às presenças oficiais da AFOS, cada uma a um canal real e atualizado:</p>
      <ul className={S.ul}>
        <li><strong>⭐ GitHub</strong>, código-fonte e histórico de commits auditável</li>
        <li><strong>LinkedIn</strong>, perfil do fundador (André Felipe) com atualizações institucionais</li>
        <li><strong>X (@AFOS_Analytics)</strong>, distribuição em tempo real de sinais e movimentos de mercado</li>
