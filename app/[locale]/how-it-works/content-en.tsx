...
export function HowItWorksEn() {
  return (
    <>
      <p className="text-center text-sm font-extrabold text-primary dark:text-blue-200 uppercase tracking-[0.25em] mb-3">The Method</p>
      <h1 className="text-3xl md:text-4xl font-extrabold text-primary dark:text-white text-center mb-2 tracking-tight">How AFOS Analytics Works</h1>
      <p className="text-center text-gray-600 dark:text-blue-100 text-base font-medium mb-10">A didactic guide to navigating the platform</p>

      <nav className="bg-white dark:bg-blue-900/40 border border-blue-100 dark:border-blue-400/40 rounded-xl p-6 my-8 shadow-sm" aria-label="Table of contents">
        <div className="text-xs font-bold text-primary dark:text-blue-200 uppercase tracking-wider mb-4">Table of Contents</div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <TocCol heading="Platform tour">
            <TocLink href="#landing">Home page (landing)</TocLink>
          ...
          <TocCol heading="Deep dive">
            ...
            <TocLink href="#glossario">Trilingual glossary</TocLink>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead>
            <tr>
              <th scope="col" className={S.th}>Source</th>
              <th scope="col" className={S.th}>What it measures</th>
              <th scope="col" className={S.th}>Why it matters</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead>
            <tr>
              <th scope="col" className={S.th}>Situation</th>
              <th scope="col" className={S.th}>Interpretation</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead>
            <tr>
              <th scope="col" className={S.th}>Variation</th>
              <th scope="col" className={S.th}>What it means</th>
              <th scope="col" className={S.th}>What to do</th>
            </tr>
          </thead>
          <tbody>
      ...
      <h2 id="landing" className={S.h2}>The home page (landing)</h2>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead><tr><th scope="col" className={S.th}>Button</th><th scope="col" className={S.th}>What it does</th></tr></thead>
          <tbody>
      ...
      <Card title="Goals modal" id="modal-metas">
        <p>Clicking <strong>Goals</strong> opens the project's strategic document: the central thesis (political information as global infrastructure), intended impact across three dimensions (economic, institutional, educational) and the country coverage roadmap.</p>
        <p>Here you'll find the project's "why": what structural problem it solves, what value it delivers to institutional investors, governments, journalists and citizens. It's the most institutionally dense page, recommended for those evaluating the platform as a partner or analytical reference.</p>
      </Card>
      ...
      <Card title="Email popup (appears during navigation)" id="email-popup">
        <p>While browsing, anyone who hasn't subscribed yet may see a discreet invitation to receive the <strong>AFOS Daily</strong> and the <strong>Tradeoff</strong> by email. It is not a header modal (it doesn't open on click): it appears on its own and is entirely <strong>optional</strong>, the whole dashboard remains 100% free and login-free.</p>
        <p><strong>Respectful behavior:</strong> it only appears after about 30 seconds on the page plus some scrolling (never right away); it closes on the X or by clicking outside; it doesn't reappear in the same session; and there is a limit of 3 appearances in total, after that never again. Subscribers stop seeing it for good.</p>
        <p><strong>Privacy:</strong> the email is used only to send the syntheses, and unsubscribing is one click away at any time.</p>
      </Card>
      ...
      <Card title="How AFOS Daily works">
        ...
        <p><strong>Editions archive:</strong> the <code>/daily</code> page lists <strong>every published edition</strong>, grouped by month with the most recent highlighted, plus a selector to <strong>jump to a specific date</strong> and the same <strong>language</strong> (PT · EN · ES) and <strong>theme</strong> (light / Sapphire Blue) selectors. Inside each synthesis, the <strong>"← Previous synthesis"</strong> and <strong>"Next synthesis →"</strong> buttons and an <strong>"All editions"</strong> link lead to the archive.</p>
      ...
      <h2 id="afos-tradeoff-card" className={S.h2}>AFOS Tradeoff, Weekly Technical Brief</h2>
      <p className={S.p}>If the AFOS Daily is the everyday read, the <strong>AFOS Tradeoff</strong> is the <strong>weekly</strong> deep dive: a technical brief published <strong>every Monday</strong>, covering the week just closed. It's built for the professional market reader (research, institutional desk, treasury, buy-side), focused on <strong>pricing and divergence</strong> rather than narrative.</p>
      <p className={S.p}>While the Daily tells the day's story, the Tradeoff measures the <strong>week's move</strong> (weekly Δ) from persisted snapshots and lays everything out in a fixed, fast-scan structure for readers who need an actionable signal.</p>

      <Card title="How the AFOS Tradeoff works">
        <p><strong>Cadence:</strong> weekly, published Mondays, covering the prior week (each edition carries a sequential number, e.g. Issue №2).</p>
        <p><strong>Fixed structure (9 blocks):</strong> executive summary (3 metric cards), "Why AFOS doesn't smooth" (why a weighted average erases signal), 3 weighted scenarios (base / contrarian to pricing / tail), indicator grid, liquidity and market structure, polling calendar, watch list of triggers, methodology, and complementary macro reading.</p>
        <p><strong>Weekly Δ:</strong> each contract shows current price, the week's move, and cumulative USD volume, computed over persisted daily snapshots, not estimates.</p>
        <p><strong>Technical tone:</strong> no journalistic flourish, no partisan adjectives, and <strong>no investment advice</strong> (every edition carries a fixed financial disclaimer). USD volume is cited as "real money" context; book liquidity is not used as an editorial signal.</p>
        <p><strong>3 languages, permalink and archive:</strong> PT-BR · EN · ES, each edition with its own URL (<code>/tradeoff/br/2026-06-01</code>). The <code>/tradeoff</code> page is the <strong>editions archive</strong>: it lists them all (by number and week), with the most recent highlighted and <strong>language</strong> and <strong>theme</strong> selectors (light / Sapphire Blue). Inside each edition, the navigation buttons and an <strong>"All editions"</strong> link lead to the archive.</p>
      </Card>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead><tr><th scope="col" className={S.th}>Candidate</th><th scope="col" className={S.th}>Poll</th><th scope="col" className={S.th}>Market</th><th scope="col" className={S.th}>Divergence</th></tr></thead>
          <tbody>
      ...
      <h3 id="contexto-estrutural" className={S.h3Anchor}>Structural context, the country's frame</h3>
      <p className={S.p}>Below the electoral signals, every validated case and the Brazil dashboard carry a <strong>Structural context</strong> block: official, open <strong>World Bank</strong> indicators that frame the country <strong>alongside</strong> the market signal, not as a predictor of it. Three groups: <strong>Governance</strong> (six Worldwide Governance Indicators, 0 to 100 scale: political stability, voice and accountability, rule of law, government effectiveness, regulatory quality, control of corruption), <strong>Economy</strong> (population, GDP, GDP per capita, inflation) and <strong>Education</strong> (public education spending and expected years of schooling). They are annual, structural indicators, they change slowly and <strong>do not predict the electoral outcome</strong>, they are background for the reading. Source: World Bank (WGI via Data360 and WDI v2), open and citable.</p>

      <h3 id="grafo-cruzamento" className={S.h3Anchor}>The cross-reference graph, the method became a navigable map</h3>
      <p className={S.p}>On every validated case and, above all, on the <strong>Brazil dashboard</strong>, the cross-reference now has an <strong>Obsidian-style force-directed graph</strong>. The election sits at the center, surrounded by the four pillars (markets, polls, press and structural context). The <strong>divergence is the star</strong>: each candidate is linked by a thin edge colored by magnitude (red = high, amber = medium, green = convergence), with the <strong>Δpp written on the edge itself</strong>. On validated cases a <strong>"real result"</strong> node points in green to whoever actually won; candidate nodes use <strong>party colors</strong>.</p>
      <p className={S.p}>On the dashboard the graph is the platform's <strong>navigable brain</strong>. Hover a node and it and its connections light up blue; click and each node takes you somewhere: <strong>data</strong> nodes open the matching folder in the open <strong>Hugging Face dataset</strong>, <strong>context</strong> nodes scroll to the on-page context card, and the <strong>shortcuts</strong> lead to the products (Daily, Tradeoff, Global, White Paper, Governance), the dashboard sections and <strong>Harvard Dataverse</strong> (collection + DOI). It is the whole method, open data, live election and platform, in a single map, in three languages.</p>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead><tr><th scope="col" className={S.th}>Institute</th><th scope="col" className={S.th}>Recent polls</th><th scope="col" className={S.th}>Average sample</th></tr></thead>
          <tbody>
      ...
      <div className="overflow-x-auto my-4">
        <table className={S.table}>
          <thead>
            <tr>
              <th scope="col" className={S.th}>Criterion</th>
              <th scope="col" className={S.th}>What it measures</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="overflow-x-auto my-4">
        <table className={S.table}>
          <thead>
            <tr>
              <th scope="col" className={S.th}>Level</th>
              <th scope="col" className={S.th}>Meaning</th>
              <th scope="col" className={S.th}>How to read polls from this institute</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead><tr><th scope="col" className={S.th}>Candidate</th><th scope="col" className={S.th}>Current poll</th><th scope="col" className={S.th}>Polymarket</th><th scope="col" className={S.th}>Trend</th></tr></thead>
          <tbody>
      ...
      <p className={S.p}>This is the footer on the <strong>dashboard and other pages</strong> (the home page has a leaner version, noted in the landing section). It is organized into <strong>four lean blocks</strong>, each with a clear purpose. No link in the footer points to an empty page, each one delivers something specific.</p>
      ...
      <ul className={S.ul}>
        ...
        <li><strong>Governance</strong>, how editorial integrity is enforced through code (automated validators + versioned rules) rather than case-by-case human review</li>
        <li><strong>Trademark</strong>, usage policy for the "AFOS Analytics" name and mark</li>
      </ul>
      ...
      <h3 className={S.h3}>Block 4, Social networks and open data</h3>
      <p className={S.pTight}>A row of icons connects to AFOS's official presences, each a real and up-to-date channel:</p>
      <ul className={S.ul}>
        <li><strong>⭐ GitHub</strong>, auditable source code and commit history</li>
        <li><strong>LinkedIn</strong>, the founder's profile (André Felipe) with institutional updates</li>
        <li><strong>X (@AFOS_Analytics)</strong>, real-time distribution of signals and market moves</li>
      ...
      <div className={S.tableWrap}>
        <table className={S.table}>
          <thead><tr><th scope="col" className={S.th}></th><th scope="col" className={S.th}>Traditional newspaper</th><th scope="col" className={S.th}>Google News</th><th scope="col" className={S.th}>AFOS</th></tr></thead>
          <tbody>
      ...
      <h2 id="glossario" className={S.h2}>Trilingual glossary of political terms</h2>
      <p className={S.p}>Brazilian politics has terms with no direct translation (TSE, STF, first round, technical tie, BolsoMaster, tarifaço) that can stall a reader from outside the country. AFOS keeps a <a href="/en/glossary" className="text-primary dark:text-yellow-400 hover:underline">dedicated glossary</a> that explains each one in <strong>3 languages</strong> (PT-BR · EN · ES).</p>
      <Card title="How the glossary is used">
        <p><strong>Its own page:</strong> at <code>/glossary</code> you find the full list, each term defined briefly and neutrally, under "Termos políticos brasileiros" (PT), "Brazilian political terms" (EN) and "Términos políticos brasileños" (ES).</p>
        <p><strong>Inline links:</strong> in the AFOS Daily and the AFOS Tradeoff, the first mention of a technical term becomes a link that takes you straight to the glossary entry, without pulling you out of your reading.</p>
        <p><strong>Why it matters:</strong> it is what makes AFOS legible to an international audience without diluting local rigor. An analyst in London or Bogotá reads the same synthesis as a voter in São Paulo, every term one click away.</p>
      </Card>
      ...
      <Card title="Step 4, Follow the AFOS Daily and the Tradeoff">
        <p><strong>AFOS Daily (every day):</strong> a ~4-minute read that explains <em>why</em> the dashboard numbers moved, with an auditable source per claim. It's what turns the day's snapshot into a story.</p>
        <p><strong>AFOS Tradeoff (every Monday):</strong> the week's technical brief, for readers who want depth on pricing and divergence (research, desk, buy-side). It brings the weekly Δ, weighted scenarios, and a watch list of triggers.</p>
      </Card>
